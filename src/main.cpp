...
int main(int argc, char *argv[]){
    ...
    if(argc<6){
        ...
        cout<<"Np: ./game --not ./additional/not.txt --and ./additional/and.txt --or ./additional/or.txt\n";
        system("PAUSE");
        exit(1);
    }
    else{
...
void readFilesAdded(int argc, char *argv[],int alternative[3][3],int conjuction[3][3],int negation[3]){
    for(int i=1;i<argc;i++){
        if(!strcmp("--not",argv[i])){
            ...
            cout<<"./game --not ./additional/not.txt --and ./additional/and.txt --or ./additional/or.txt";
            exit(1);
        }
    }
}
